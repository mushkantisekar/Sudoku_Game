...
function printPuzzle(n)
		{
			...
			text+="</table><br><br><br>&nbsp;&nbsp;&nbsp;<button type='button' onclick='check(this.form);' class='btn btn-danger btn-lg'>Submit</button>&nbsp;&nbsp;<button type='button' onclick='quit();' class='btn btn-danger btn-lg'>Give up</button></form>";
			document.getElementById("id01").innerHTML=text;
			show();
...
function check(form)
		{
			...
			if(!checkSudoku())
				text = "<p style='font-size:150%;color:red'>INCORRECT ANSWER!<br>CORRECT ANSWER IS :</p><br>";
			printAns(text,arr);
		}
function checkSudoku()
{
	...
	for(i=0;i<9;i++)
	{
		...
		{
			val=arr1[i][j];
			if(!checkRow(i,val)||!checkCol(j,val)||!checkBlock(i,j,val)||val==0)
				return false;
		}
	}
	return true;
